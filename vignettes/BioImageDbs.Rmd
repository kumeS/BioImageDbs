---
title: "Provide Bioimage Dataset for ExperimentalHub"
author: 
- name: Satoshi Kume
  affiliation:
...
date: "`r Sys.Date()`"
graphics: no
package: ExperimentalHub, knitr, magick
output:
  BiocStyle::html_document:
  toc_float: true
vignette: >
  %\VignetteIndexEntry{Provide Bioimage Dataset for ExperimentalHub}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
  %\VignetteDepends{ExperimentalHub}
---
...
After getting the data from ExperimentalHub, it can be used for deep learning 
...
ExperimentalHub is also intended for applications such as (1) comparing models 
...
Next we list all BioImageDbs entries from `ExperimentalHub`.

```{r list-BioImageDbs}
query(ah, "BioImage")
```

We can confirm the metadata in ExperimentalHub in Bioconductor S3 bucket
with `mcols()`.
...
# 5D Arrays from the ExperimentalHub
...
# 4D Arrays from the ExperimentalHub
...
# Visualization of gif images from the ExperimentalHub
